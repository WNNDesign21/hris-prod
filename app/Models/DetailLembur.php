...
class DetailLembur extends Model
{
    ...
    public static function getReportMonthlyPerDepartemen($month, $year)
    {
        $data = self::selectRaw('
            ...
            SUM(detail_lemburs.nominal) as gaji_lembur,
            SUM(CASE 
            ...
            SUM(CASE 
                    WHEN posisis.jabatan_id >= 5 AND detail_lemburs.durasi >= 4 THEN 15000 
                    ELSE 0 
                END) + SUM(detail_lemburs.nominal) as total_gaji_lembur
        ')
        ...
        ->groupBy('karyawans.nama', 'departemens.nama','detail_lemburs.departemen_id', 'posisis.nama', 'setting_lembur_karyawans.gaji')
    ...
    //                 ELSE
    //                     (CASE 
    //                         WHEN detail_lemburs.durasi > 0 THEN TRUNC(TRUNC(1 * 1.5) + TRUNC(TRUNC(TRUNC(detail_lemburs.durasi) - 1) * 2)) / 60
    //                     ELSE 0
    //                     END)
    //                 END)
    //             ), 2
